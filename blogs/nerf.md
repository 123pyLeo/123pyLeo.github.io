...
在相机坐标系下，点的齐次坐标为[x_c, y_c, z_c, 1]，但在透视投影时，我们主要关注前三维[x_c, y_c, z_c]。

通过相机内参矩阵K进行变换：
Q = K * [x_c, y_c, z_c] = [f_x, 0, c_x; 0, f_y, c_y; 0, 0, 1] * [x_c, y_c, z_c] = [f_x*x_c + c_x*z_c, f_y*y_c + c_y*z_c, z_c]

为了得到非齐次的图像平面坐标，需要进行归一化：
[u, v, 1] = (1/z_c) * [f_x*x_c + c_x*z_c, f_y*y_c + c_y*z_c, z_c]
